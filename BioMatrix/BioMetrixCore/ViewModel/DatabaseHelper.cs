...
namespace BioMetrixCore.ViewModel
{
    ...
    public class DatabaseHelper : IDisposable
    {
        ...
        /// DLGenerator		6/25/2012 4:26:00 PM			Created function
        /// 
        /// </RevisionHistory>
        /// 
        /// </remarks>
        ...
        {
            databaseToUse = SupportedDatabases.OleDb;
        ...
        //ExecuteScalarEmbedded


        public int ExecuteScalarEmbedded(string sqlStatement, ref bool executionSucceeded)
        {
            return ExecuteNonQuery(sqlStatement, CommandType.Text, ConnectionState.CloseOnExit, ref executionSucceeded);
        }


        //ExecuteScalarEmbedded

        public DbDataReader ExecuteReaderEmbedded(string sqlStatement, ref bool executionSucceeded)
        {
            return ExecuteReaderUsingSQL(sqlStatement, ref executionSucceeded);
        }
                ...
                try
                {
                    if (configFileAndPath.Trim().Length == 0)
                    {
                        ...
                        if (!System.IO.File.Exists(configFileAndPath))
                        {
                            ...
                            configFileAndPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory + "bin", "Ignyte.DataLayer.config");
                            if (File.Exists(configFileAndPath) == false)
                                configFileAndPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Ignyte.DataLayer.config");
                    ...
                    if (System.IO.File.Exists(configFileAndPath))
                    {
                        ...
                        _connectionInfo.ConnectionString = keyToUse; //configFile.ConnectionStrings.ConnectionStrings[keyToUse].ConnectionString;
                        _connectionInfo.ProviderName = "System.Data.SqlClient"; //configFile.ConnectionStrings.ConnectionStrings[keyToUse].ProviderName;
                    }
                    else
                    {
